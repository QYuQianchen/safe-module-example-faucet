...
import { Safe, OwnerManager, ModuleManager, Enum } from "safe-contracts/contracts/Safe.sol";
...
contract DeploymentScript is NetworkUtilsScript {
        ...
        returns (address[] memory safes)
    {
        ...
        for (uint256 i = 0; i < safes.length; i++) {
            ...
            bytes memory enableModuleData = abi.encodeWithSelector(ModuleManager.enableModule.selector, faucetModule);
     ...
     * @return faucetModule address of faucet module
     */
    function deployModule() public returns (address faucetModule) {
        // deploy faucet module
        faucetModule = deployCode("FaucetModule.sol:FaucetModule");
        emit log_named_address("Faucet module", faucetModule);
    }
